...
function checkCustomerName(username){
	createXMLHttp(); //建立xmlHttp核心对象
	xmlHttp.open("POST", "CheckUserName?username="+username);//设置一个请求，通过地址重写的方式将username传递到jsp页面
	//设置请求完成以后的回调函数
	xmlHttp.onreadystatechange=checkCustomerNameCallBack;
	xmlHttp.send(null);
	document.getElementById("msg").innerHTML="正在验证---";
}
function checkCustomerNameCallBack(){
	 if(xmlHttp.readyState==4){
		 if(xmlHttp.status==200){
			 var text=xmlHttp.responseText;
			 if(text=="true"){
				 flag=false;
				 document.getElementById("msg").innerHTML="此用户被占用,无法使用";
			 }else{
				 flag=true;
