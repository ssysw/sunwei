...
<!--文件体开始-->
		...
		<tr>
         ...
         <c:forEach var="entry" items="${shoppingCar.lines}">      
		<tr>
			<form method="post" action="user/updateShoppingCartNum" name="f1">
			<input type="hidden" name="bookid" value="${entry.value.book.id}">
			...
			<input type="button" value="取消" onclick="javascript:window.location='user/removerShoppingLine?bookid=${entry.value.book.id}';"> 
		...
		<tr>
			...
			<input type="button" value="提交订单" onclick="javascript:window.location='user/confirmOrder.jsp';">
			 ...
			 <input type="button" value="清空购物车" onclick="javascript:window.location='clearShoppingCart';"></td>
		</tr>
                </table><br>
<!--文件尾开始-->
