...
<!--文件体开始-->
          <script type="text/javascript">
           var msg = ${deleteOderMsg};
          if(msg){
        	  alert(msg);
          }
          </script>
          <c:remove var="deleteOderMsg" scope="session"/>
		...
		<tr>
          ...
          <c:forEach var="order" items="${requestScope.orders}" varStatus="a" step="1" begin="0">    
		<tr>
			<td class=tablebody2 valign=middle align=center width="">${a.count}</td>
			...
			<input type="button" value="删除" onclick="javascript:window.location='user/deleteOrder?orderid=${order.id}';">&nbsp;
			<input type="button" value="明细" onclick="javascript:window.location='user/showLines?orderid=${order.id}';"><!--&nbsp;<input type="button" value="修改"/>--> </td>
		</tr>
		</c:forEach>  
        </table><br>
